...
import { Download, Upload, Globe, Moon, Sun, Bell, Database, LogOut, AlertTriangle, ChevronDown } from 'lucide-react';
        ...
        <div className="space-y-6 lg:space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-6">
              {/* Language Toggle */}
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">{t('language' as any) || 'Language'}</label>
                <div className="flex bg-slate-50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-100 dark:border-slate-700/50">
                  <button 
                    ...
                    className={cn(
                      ...
                      lang === 'en' ? "bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                    )}
                  >
                    English
                  </button>
                  <button 
                    ...
                    className={cn(
                      "flex-1 py-2.5 text-sm font-bold rounded-lg transition-all",
                      lang === 'fa' ? "bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                    )}
                  >
                    فارسی
                  </button>
                            ...
                            {/* Theme Picker */}
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">{t('theme' as any) || 'Theme'}</label>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
                This default time is used for notifications if a habit doesn't have a custom reminder set. You can override this setting individually when creating or editing a habit.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="pt-4 mt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                >
                  <div className="flex flex-col text-left">
                    ...
                    <span className="text-xs font-medium text-red-400/80 dark:text-red-500/80">Permanently delete all habits and history</span>
                  </div>
                </button>
              </div>
            </div>
