...
export default function HabitCard({ habit, dayRecords }: HabitCardProps) {
	...
	return (
		<div className='bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-50 dark:border-slate-800 shadow-sm relative overflow-hidden transition-all'>
			<div className='flex justify-between items-start mb-6'>
				<div className='flex gap-4 items-center'>
					...
					<div>
						{categoryDef && (
							<span
								className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold mb-1 uppercase tracking-wider ${categoryDef.color
									.split(" ")
						...
						{habit.description && <p className='text-slate-500 dark:text-slate-400 text-sm line-clamp-1'>{habit.description}</p>}
					</div>
				</div>
				<button
				...
				{targetDates.map((date, index) => {
					...
					return (
						<button
							...
							{hasNote && <div className='absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-amber-400' />}
						</button>
					);
				})}
			</div>
						...
						{dayRecords
							...
							.map((record) => (
								<div
									...
									<div
										className='text-slate-700 dark:text-slate-200 text-sm prose prose-sm dark:prose-invert max-w-none'
										dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(record.note || "") }}
									/>
								</div>
							))}
					</div>
			...
			{isEditModalOpen && <HabitModal habit={habit} onClose={() => setIsEditModalOpen(false)} />}
